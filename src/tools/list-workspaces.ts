 ...
 *
 * Lists every workspace this connector can see and flags the one it is pinned
 * to. It cannot switch workspaces; the user reconnects and picks a different
...
export async function handleListWorkspaces(
  ...
  try {
    ...
    return `Workspaces you can see:\n\n${lines.join("\n")}\n\nThis connector reads the active workspace. To query a different one, reconnect and choose it during authorization.`;
  } catch (error) {
...
export const listWorkspacesTool = {
  ...
  description:
    "List the workspaces this connector can see and show which one it is reading. It cannot switch workspaces; if the user asks about a different workspace, tell them to reconnect and choose it.",
  inputSchema: {
    type: "object",
    properties: {},
    required: []
