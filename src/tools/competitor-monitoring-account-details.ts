...
export const competitorMonitoringAccountDetailsTool = {
  ...
  description:
    ...
    "`crmCompanies` links the company's CRM records (its own record is `primary`; the others are CRM records judged to be the same company), and each opportunity's `matchedVia` names the same-company domain it sits on when it is not on the company's own record. " +
    ATTRIBUTION_NOTE +
