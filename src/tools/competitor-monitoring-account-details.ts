...
export const competitorMonitoringAccountDetailsTool = {
  ...
  description:
    "Fetch everything known about one company in the competitor monitoring table: the CRM outreach rollup (whether outreach started, whether they replied, the in-market verdict and summary), the per-contact outreach records, the recent CRM activity timeline where each entry carries its reply classification and sentiment, the competitor signals we surfaced and when, the live CRM opportunities for the company, and the extended buying committee — the decision-makers discovered at the account beyond the people who engaged a competitor. Identify the company with company_key (from get_competitor_monitoring_table), company_domain, or company_name. A company that is not in the table returns found: false rather than an error. Use `include` to fetch only the blocks you need and activity_limit to cap the timeline. " +
    ATTRIBUTION_NOTE +
