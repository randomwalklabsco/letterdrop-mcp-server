...
export const competitorMonitoringTableTool = {
  ...
  description:
    "Fetch the full competitor monitoring table. It is the single account-grained read behind every competitor-table, all-leads, decision-maker, CSV, export, report and recommended-outreach question. It returns one row per company with both contact groups by default: `contacts` contains people who personally produced a competitor signal, and `buyingCommittee` contains the extended decision-makers found at the same account. There is no separate per-lead or 'Show buying committee' tool: the committee is part of this response unless the caller removes `buying_committee` from `include`. Each person carries `recommendedAction` and `recommendedChannels` with the workspace's weekly capacity-based outreach allocation. These fields are action guidance, not an enriched email address or a named sequence. Each account also carries the competitors it engaged, priority (high/medium/low/nurture), staleness, guessed sales-cycle stage and reasoning, CRM deal status / stage / amount / owner / open date / close date, outreach status and CRM summary, custom CRM columns, signal dates, and contacted/replied state. Supports free-text search, filters, sorting, and limit-offset pagination. Follow `pagination.hasMore` and call again with `pagination.nextOffset` until false when the user asks for the entire dataset. Start at limit 25 and narrow with filters rather than paging deeply — each page is a full table read. Note `existingAccount` is null when the CRM lookup was unavailable, which is NOT the same as net-new. " +
    BUDGETED_RECOMMENDATION_NOTE +
    " " +
    GUESSED_STAGE_NOTE +
    " " +
